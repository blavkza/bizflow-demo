  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center py-8">
        <div className="text-muted-foreground text-sm">Loading call out records...</div>
      </div>
    );
  }
    ...
    return (
      <Card>
        <CardContent className="flex flex-col items-center justify-center py-12">
          ...
          <h3 className="text-lg font-semibold mb-2">No emergency call outs found</h3>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center space-x-4 flex-1">
                  <Avatar>
                    <AvatarImage src={person?.avatar || "/placeholder.svg"} alt={personName} />
                    <AvatarFallback>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center space-x-2">
                      ...
                      <span className="text-xs text-muted-foreground">{personNumber}</span>
                    </div>
                    <p className="text-sm font-medium text-destructive mt-0.5">{record.title}</p>
                    {record.message && (
                    ...
                    <div className="flex items-center gap-3 mt-1.5">
                      <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                        ...
                        <span>Requested: {new Date(record.requestedAt).toLocaleString()}</span>
                      </div>
                      <div className="flex items-center gap-1 text-[10px] text-muted-foreground">
                        <User className="h-3 w-3" />
                        <span>By: {record.requestedUser?.name || "System"}</span>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex items-center space-x-6">
                  ...
                  <div className="text-center w-24">
                    ...
                    <p className="text-[10px] text-muted-foreground">
                      {new Date(record.startTime).toLocaleDateString([], { day: '2-digit', month: 'short' })}
                    </p>
                  </div>

                  <div className="text-center w-24">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider text-emerald-600">Check In</p>
                    <p className="font-medium text-xs">
                      {record.checkIn ? formatTime(record.checkIn) : <span className="text-muted-foreground">-</span>}
                    </p>
                    {record.checkInAddress && (
                      <HoverCard>
                        <HoverCardTrigger asChild>
                          <div className="flex items-center justify-center text-[10px] text-muted-foreground mt-0.5 cursor-pointer">
                            <MapPin className="w-2.5 h-2.5 mr-0.5" />
                            <span className="truncate max-w-[60px]">{record.checkInAddress}</span>
                          </div>
                        </HoverCardTrigger>
                  ...
                  <div className="text-center w-24">
                    <p className="text-[10px] text-muted-foreground uppercase tracking-wider text-emerald-600">Check Out</p>
                    <p className="font-medium text-xs">
                      {record.checkOut ? formatTime(record.checkOut) : <span className="text-muted-foreground">-</span>}
                    </p>
                  </div>
                  ...
                  {record.duration && (
                    <div className="text-center w-20">
                      ...
                      <p className="font-bold text-xs text-primary">{record.duration.toFixed(1)}h</p>
                    </div>
                  )}
                </div>
              </div>
