...
export function EmploymentDetailsCard({
  ...
  const getWorkingDays = (): string[] => {
    if (!freelancer.workingDays) return [];

    try {
      if (Array.isArray(freelancer.workingDays)) {
        return freelancer.workingDays;
      }

      if (typeof freelancer.workingDays === "string") {
        return JSON.parse(freelancer.workingDays);
      }
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {freelancer.department?.manager?.name || "Not assigned"}
          </p>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium text-green-700">
                {formatTime(freelancer.scheduledKnockIn)}
              </p>
            </div>
            <div>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {freelancer.position || "Not specified"}
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {freelancer.department?.name || "Not assigned"}
          </p>
        </div>
      </CardContent>
    </Card>
