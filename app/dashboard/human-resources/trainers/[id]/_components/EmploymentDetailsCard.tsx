...
export function EmploymentDetailsCard({
  ...
  const getWorkingDays = (): string[] => {
    if (!trainer.workingDays) return [];

    try {
      if (Array.isArray(trainer.workingDays)) {
        return trainer.workingDays;
      }

      if (typeof trainer.workingDays === "string") {
        return JSON.parse(trainer.workingDays);
      }
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center justify-between">
          <div className=""> Trainer Details</div>
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {trainer.department?.manager?.name || "Not assigned"}
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4 my-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {trainer.terminationDate
                ? new Date(trainer.terminationDate).toLocaleDateString()
                : "Not Assigned"}
            </p>
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              ...
              <p className="font-medium text-green-700">
                {formatTime(trainer.scheduledKnockIn)}
              </p>
            </div>
            <div>
              <p className="text-muted-foreground">Knock Out</p>
              <p className="font-medium text-red-500">
                {formatTime(trainer.scheduledKnockOut)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <p className="text-muted-foreground">Knock In</p>
              <p className="font-medium text-green-700">
                {formatTime(trainer.scheduledWeekendKnockIn)}
              </p>
            </div>
            <div>
              <p className="text-muted-foreground">Knock Out</p>
              <p className="font-medium text-red-500">
                {formatTime(trainer.scheduledWeekendKnockOut)}
              </p>
            </div>
          </div>
        </div>
        ...
        <div>
          ...
          <p className="text-sm  font-semibold">
            R {(Number(trainer.overtimeHourRate) || 0).toFixed(2)} per hour
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {trainer.position || "Not specified"}
          </p>
        </div>
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {trainer.department?.name || "Not assigned"}
          </p>
        </div>
      </CardContent>
    </Card>
