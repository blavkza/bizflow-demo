...
import { Department, Trainer, TrainerStatus } from "@prisma/client";
...
import { trainerSchema, trainerSchemaType } from "@/lib/formValidationSchemas";
...
export default function TrainerForm({
  ...
  const onSubmit = async (values: trainerSchemaType) => {
    setIsLoading(true);
    try {
      const formattedData = {
        ...values,
      ...
      if (type === "create") {
        await axios.post("/api/trainers", formattedData);
        toast.success("Trainer created successfully");
      } else if (type === "update" && data?.id) {
        await axios.put(`/api/trainers/${data.id}`, {
          ...formattedData,
          avatar: data.avatar,
        });
            ...
            render={({ field }) => (
              <FormItem>
                ...
                >
                  ...
                  <SelectContent>
                    {Object.values(TrainerStatus).map((status) => (
            ...
            render={({ field }) => (
              <FormItem className="flex flex-row items-start space-x-3 space-y-0 rounded-md border p-4">
                ...
                <div className="space-y-1 leading-none">
                  <FormLabel>Reliable Trainer</FormLabel>
                  <div className="text-sm text-muted-foreground">
                    Mark this trainer as reliable for future assignments
                  </div>
                </div>
              </FormItem>
            )}
          ...
          render={() => (
            <FormItem className="border-t pt-6">
              <div className="mb-4">
                ...
                <div className="text-sm text-muted-foreground">
                  Select the days this trainer is scheduled to work
                </div>
              </div>
