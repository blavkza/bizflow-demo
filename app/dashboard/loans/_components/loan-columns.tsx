...
export type LoanColumn = {
  ...
  calculationMethod: "AMORTIZED" | "FLAT";
...
export const columns: ColumnDef<LoanColumn>[] = [
  ...
  },
  {
    ...
    cell: ({ row }) => {
        ...
        const totalPaid = (loan.payments || []).reduce((acc: number, curr: any) => acc + curr.amount, 0);
        ...
        const progress = Math.min(100, Math.max(0, (totalPaid / finalTotalPayable) * 100));

        return (
            <div className="w-[120px]">
                <div className="flex justify-between text-sm mb-1">
                    <span className={balance <= 0 ? "text-green-600 font-bold" : "font-medium"}>
  ...
  },
  {
    ...
    cell: ({ row }) => {
        ...
        const variant = 
            ...
            status === "PAID_OFF" ? "outline" : // Was success/green in mind but outline is safe
            status === "DEFAULTED" ? "destructive" : "secondary";
            
        const colorClass = 
            status === "ACTIVE" ? "bg-blue-100 text-blue-800 hover:bg-blue-100" :
            status === "PAID_OFF" ? "bg-green-100 text-green-800 hover:bg-green-100 border-green-200" :
            status === "DEFAULTED" ? "bg-red-100 text-red-800 hover:bg-red-100" :
            "bg-gray-100 text-gray-800";

      return <Badge className={colorClass} variant="outline">{status.replace("_", " ")}</Badge>;
    },
  },
  {
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            <DropdownMenuItem asChild>
                 <Link href={`/dashboard/loans/${loan.id}`} className="flex items-center cursor-pointer">
                    <Eye className="mr-2 h-4 w-4" /> View Details
                 </Link>
            </DropdownMenuItem>
             <DropdownMenuItem asChild>
                 <Link href={`/dashboard/loans/${loan.id}?edit=true`} className="flex items-center cursor-pointer">
